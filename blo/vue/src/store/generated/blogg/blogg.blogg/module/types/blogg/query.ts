...
import { Params } from "../blogg/params";
import {
  PageRequest,
  PageResponse,
} from "../cosmos/base/query/v1beta1/pagination";
import { Post } from "../blogg/post";
import { Comment } from "../blogg/comment";
...
export const QueryPostsResponse = {
  encode(
    ...
    if (message.approved !== "") {
      writer.uint32(50).string(message.approved);
    }
    return writer;
  },
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("blogg.blogg.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new Reader(data)));
  }

  Posts(request: QueryPostsRequest): Promise<QueryPostsResponse> {
    const data = QueryPostsRequest.encode(request).finish();
    const promise = this.rpc.request("blogg.blogg.Query", "Posts", data);
  ...
  Comment(request: QueryGetCommentRequest): Promise<QueryGetCommentResponse> {
    ...
    const promise = this.rpc.request("blogg.blogg.Query", "Comment", data);
  ...
  CommentAll(
    ...
    const promise = this.rpc.request("blogg.blogg.Query", "CommentAll", data);
    return promise.then((data) =>
  ...
  Comments(request: QueryCommentsRequest): Promise<QueryCommentsResponse> {
    ...
    const promise = this.rpc.request("blogg.blogg.Query", "Comments", data);
    return promise.then((data) =>
